<?php
    session_start();
    require 'connection.php';
    if(!isset($_SESSION['email'])){
        header('location: login.php');
    }
    $user_id=$_SESSION['id'];
    $user_products_query="select it.id,it.name,it.price from users_items ut inner join items it on it.id=ut.item_id where ut.user_id='$user_id'";
    $user_products_result=mysqli_query($con,$user_products_query) or die(mysqli_error($con));
    $no_of_user_products= mysqli_num_rows($user_products_result);
    $sum=0;
    if($no_of_user_products==0){
        //echo "Add items to cart first.";
    ?>
        <script>
        window.alert("No items in the cart!!");
        </script>
    <?php
    }else{
        while($row=mysqli_fetch_array($user_products_result)){
            $sum=$sum+$row['price']; 
       }
    }
    $id=$_SESSION['id'];
    $address = $_POST['addr'];
    $pin = $_POST['pin'];
    $city = $_POST['city'];
    $state = $_POST['state'];
    if(isset($_POST['submit']))
{
    $sql1 = "INSERT INTO transaction (user_id,address,pin,city,state) VALUES ('$id','$address','$pin','$city','$state')";
    $result1 = mysqli_query($con,$sql1);
}
    ?>
<!DOCTYPE html>
<html>
<head>
	<link rel="shortcut icon" href="img/lifestyleStore.png" />
        <title>Online Shopping Store</title>
        <meta charset="UTF-8">
        <meta name="viewport" content="width=device-width, initial-scale=1.0">
        <!-- latest compiled and minified CSS -->
        <link rel="stylesheet" href="bootstrap/css/bootstrap.min.css" type="text/css">
        <!-- jquery library -->
        <script type="text/javascript" src="bootstrap/js/jquery-3.2.1.min.js"></script>
        <!-- Latest compiled and minified javascript -->
        <script type="text/javascript" src="bootstrap/js/bootstrap.min.js"></script>
        <!-- External CSS -->
        <link rel="stylesheet" href="css/style.css" type="text/css">
</head>
<body>
	<div>
            <?php 
               require 'header.php';
            ?>
            <br>

</div>
<h2>Order Successfull!!!!!!!</h2>
<h3>Shipping Address</h3>
<div style="padding: 20px">
	
	<p><?php echo $address; ?></p>
	<p><?php echo $city; ?></p>
	<p><?php echo $state; ?></p>
	<p><?php echo $pin; ?></p>
</div>
<h3>Products</h3>
<div class="container">
                <table class="table table-bordered table-striped">
                    <tbody>
                        <tr>
                            <th>Item Name</th><th>Price</th>
                        </tr>
                       <?php 
                        $user_products_result=mysqli_query($con,$user_products_query) or die(mysqli_error($con));
                        $no_of_user_products= mysqli_num_rows($user_products_result);
                        $counter=1;
                       while($row=mysqli_fetch_array($user_products_result)){
                           
                         ?>
                        <tr>
                            <th><?php echo $row['name']?></th><th><?php echo $row['price']?></th>
                           
                        </tr>
                       <?php $counter=$counter+1;}?>
                        <tr>
                            <th>Total</th><th>Rs <?php echo $sum;?>/-</th>
                        </tr>
                    </tbody>
                </table>
            </div>
</body>
</html>